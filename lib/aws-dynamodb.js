(function() {
    ...
    ItemListExpression.prototype.parse = function(itemList) {
        ...
        attributeItems.forEach(function(attributeItem) {
            ...
            if(attributeItem.length != 3) {
                console.error("Error: Invalid format.", expr);
                process.exit(1);
            } else if(attributeItem[0].type != "ident") {
                console.error("Error: Lvalue must be a item name or map path.", expr);
                process.exit(1);
            } else if(attributeItem[1].type != "comp" || attributeItem[1].lex != "=") {
                console.error("Error: The expression must be an assignment.", expr);
                process.exit(1);
            } else if(attributeItem[2].type != "bool" &&
                    attributeItem[2].type != "str" &&
                    attributeItem[2].type != "num")
            {
                console.error("Error: Rvalue must be a value.", expr);
                process.exit(1);
            }
    ...
    //      }
    //
    awscli.dynamodb.parseItemListToMap = function(itemList) {
